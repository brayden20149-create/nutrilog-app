...
// Group a day's flat workout entries by exercise name → { name, sets[], volume, topWeight, totalReps }
export const groupExercises = (list=[]) => {
  const map = {};
  for (const w of list) {
    const name = (w.name||"").trim();
    if (!map[name]) map[name] = { name, sets:[], volume:0, topWeight:0, totalReps:0, category:w.category };
    const p = parseSet(w.detail||"");
    map[name].sets.push({ ...w, parsed:p });
    if (p) { map[name].volume += p.volume; map[name].topWeight = Math.max(map[name].topWeight, p.weight); map[name].totalReps += p.reps; }
  }
...
export const analyzeWorkoutDay = (workouts, dayKey) => {
  ...
  // Most-recent prior performance per exercise name
  const lastByName = {};
  for (const k of priorKeys) {
    for (const g of groupExercises(workouts[k])) {
      lastByName[g.name] = g; // later keys overwrite → ends as most recent
    }
  }
  ...
  for (const k of priorKeys) {
    for (const g of groupExercises(workouts[k])) {
      const b = bestByName[g.name] || { topWeight:0, volume:0 };
      bestByName[g.name] = { topWeight:Math.max(b.topWeight,g.topWeight), volume:Math.max(b.volume,g.volume) };
    }
  }
  ...
  const exercises = todayGroups.map(g=>{
    const badges = [];
    const prev = lastByName[g.name];
    const best = bestByName[g.name];
    // Weight PR
