...
// Normalize an exercise name so minor variations match: lowercase, collapse
// spaces, strip punctuation, and singularize a trailing plural.
export const normName = (name="") => {
  let n = name.toLowerCase().trim().replace(/[^a-z0-9 ]/g,"").replace(/\s+/g," ");
  // singularize the last word if it ends in 's' (curls→curl, pulldowns→pulldown)
  n = n.replace(/s\b/g,"");
