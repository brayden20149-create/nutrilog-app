    ...
    try {
      ...
      if (actions.length) {
        ...
        actions.filter(a=>a.type==="save_program" && a.program).forEach(a=>{
          const rid = () => Date.now().toString(36)+Math.random().toString(36).slice(2);
          // Normalize each exercise's sets into an array of {id,weight,reps}
          const normSets = (e) => {
            if (Array.isArray(e.sets)) {
              return e.sets.map(s=>({ id:rid(), weight:s.weight||"", reps:(s.reps!=null?String(s.reps):"") }));
            }
            // Legacy: sets is a count, with shared reps/weight → expand into N sets
            const n = Math.max(1, +e.sets||1);
            return Array.from({length:n}).map(()=>({ id:rid(), weight:e.weight||"", reps:(e.reps!=null?String(e.reps):"") }));
          };
      ...
      {activeTab==="workouts"&&(
        <div style={{flex:1,overflowY:"auto",padding:"12px 14px",WebkitOverflowScrolling:"touch"}}>
          ...
          {dayWorkouts.length>0 ? (() => {
            ...
            const exMap = {};
            (analysis.exercises||[]).forEach(e=>{ exMap[(e.name||"").trim().toLowerCase()] = e; });
            const groups = [];
            const idx = {};
            ...
            return (
              <>
                ...
                {groups.map((grp,gi)=>{
                  const ex = exMap[(grp.name||"").trim().toLowerCase()];
                  ...
                  <div key={gi} style={{background:T.card,borderRadius:12,
                    ...
                    {grp.sets.map((w,si)=>{
                      ...
                      <div key={w.id} style={{display:"flex",alignItems:"center",
                        ...
                        <div style={{flex:1,minWidth:0,fontSize:13,color:isTop?T.warn:T.text,
