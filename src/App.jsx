...
export default function App() {
  ...
  const applyActions = (actions, curGoals, curEntries) => {
    let g=[...[]],gl={...curGoals},es=[...curEntries];
    ...
    return {newGoals:gl,newEntries:es};
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const didChange=actions.some(a=>["add_entry","remove_entry","clear_log"].includes(a.type));
      if (didChange) {
        setActiveTab("log");
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex",margin:"10px 14px 0",background:T.surface,
        ...
        {[["chat","💬 Chat"],["log","📋 Log"]].map(([tab,label])=>(
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",
          ...
          <div style={{flexShrink:0,paddingBottom:`max(env(safe-area-inset-bottom),14px)`,paddingTop:8}}>
            <div style={{display:"flex",gap:8,alignItems:"flex-end",
              ...
              <button onClick={handleSend} disabled={loading||!input.trim()}
                style={{background:T.accent,color:"#0b0f0b",border:"none",
                  ...
                  minHeight:44,minWidth:64,cursor:loading||!input.trim()?"not-allowed":"pointer",
                  opacity:loading||!input.trim()?0.4:1,transition:"opacity .15s",flexShrink:0,
            ...
            <div style={{fontSize:11,color:T.muted,textAlign:"center",marginTop:7,lineHeight:1.5}}>
              "log a Chick-fil-A sandwich" · "how's my protein?" · "remove the fries"
            </div>
          </div>
        </div>
      )}
          ...
          {entries.length>0 ? (<>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:11,color:T.accent,letterSpacing:"0.12em"}}>
                {isToday(selDay)?"TODAY'S LOG":fmtDate(selDay).toUpperCase()+" LOG"} · {entries.length} ITEMS
              </div>
            ...
            <div style={{textAlign:"center",padding:"40px 0",color:T.muted}}>
              ...
              <div style={{fontSize:15,marginBottom:6}}>
                {isToday(selDay)?"Nothing logged yet.":"No entries for this day."}
              </div>
