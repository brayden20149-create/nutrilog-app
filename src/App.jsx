...
async function callTrainer(messages) {
  const SYSTEM = [
    "You are a strength coach inside a fitness app. You talk like an experienced training partner who knows the science, not a cheerleader.",
    "TONE: Direct, technical, matter-of-fact. Do NOT use hype, excessive praise, or flattery ('crushing it', 'beast mode', 'amazing job', emojis everywhere). Also do NOT be harsh, demeaning, or drill-sergeant. Treat the user as a capable adult. Neutral, useful, a little dry is the target.",
    "WHAT TO DO when the user reports a lift: 1) If their workout history is provided, compare this session to previous performance on the same lift (load, volume, reps) and state the change plainly. 2) Give 1-2 concrete mental/technical cues for that lift (e.g. 'spread the floor', 'pull the bar into you on the descent', 'brace before you unrack'). 3) If relevant, give a brief, specific suggestion for next session (small load or rep target), grounded in progressive overload — without being pushy.",
    "Keep answers concise and skimmable. A few short sentences or a couple of short bullet lines. No long lectures unless they ask for depth.",
    "If they ask general training questions (programming, form, recovery, nutrition timing), answer technically and honestly, noting tradeoffs. Don't pretend certainty you don't have.",
    "If a USER PROFILE is provided (age, sex, weight, training experience, days/week, goal), factor it into your feedback — e.g. scale expectations and progression to their experience level and weekly frequency, and align advice with their stated goal (cut/bulk/maintain).",
    "You don't log anything — another part of the app handles logging. You're here to coach and give feedback. Reply in plain text only.",
  ].join(" ");
  ...
  const data = await res.json();
  const raw = (data.content || []).map(b => b.text || "").join("").trim();
  return raw || "I didn't catch that — give me the lift and the numbers.";
}
...
export default function App() {
  ...
  const [trainerMsgs, setTrainerMsgs] = useState([{
    role:"assistant",
    content:"Strength coach here. Tell me what you trained — lift, sets, reps, load — and I'll give you feedback, cues, and how it stacks up against last time. Or ask me anything technical.",
  }]);
  ...
  useEffect(()=>{
    const writeSnapshot = () => {
      try {
        _set("nl4_snapshot", JSON.stringify({ days:allDays, goals, meals, workouts, profile, ts:Date.now() }));
      } catch {}
    };
    writeSnapshot();
  ...
  },[allDays, goals, meals, workouts, profile]);
  ...
  const handleTrainerSend = async () => {
    ...
    setTrainerMsgs(prev=>[...prev,{role:"user",content:text}]);
    ...
    apiMsgs.push({role:"user",content:text+histBlock+profBlock});

    try {
      const reply = await callTrainer(apiMsgs);
      setTrainerMsgs(prev=>[...prev,{role:"assistant",content:reply}]);
    } catch(err) {
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex",margin:"10px 14px 0",background:T.surface,
        ...
        {[["chat","💬"],["log","📋"],["meals","🍱"],["train","🏋️"],["week","📅"],["profile","👤"]].map(([tab,label])=>(
          ...
          <div style={{flexShrink:0,paddingBottom:`max(env(safe-area-inset-bottom),14px)`,paddingTop:8}}>
            <div style={{display:"flex",gap:8,alignItems:"flex-end",
              ...
              <textarea ref={trainerInputRef} value={trainerInput} rows={1}
                ...
                onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();handleTrainerSend();}}}
                placeholder="e.g. squat 5x5 at 245, felt heavy on rep 4"
                style={{flex:1,background:"none",border:"none",color:T.text,
                  fontSize:16,lineHeight:1.5,padding:"4px 0",outline:"none",
                  overflowY:"auto",minHeight:26,maxHeight:88}}/>
              <button onClick={handleTrainerSend} disabled={trainerLoading||!trainerInput.trim()}
                style={{background:T.info,color:"#0b0f0b",border:"none",borderRadius:12,
                  ...
                  cursor:trainerLoading||!trainerInput.trim()?"not-allowed":"pointer",
                  opacity:trainerLoading||!trainerInput.trim()?0.4:1,transition:"opacity .15s",
            ...
            <div style={{fontSize:11,color:T.muted,textAlign:"center",marginTop:7}}>
              Logs happen in Chat — this coach gives feedback & cues.
            </div>
          </div>
        </div>
      )}
