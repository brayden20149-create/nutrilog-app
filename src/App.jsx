...
async function callTrainer(messages) {
  const SYSTEM = [
    ...
    '{"message":"your coaching reply","actions":[],"workoutStatus":"none|partial|complete"}',
    "workoutStatus: set to 'complete' if what they logged this session reads like a finished workout (multiple exercises, or a clearly complete session for their split/goal); 'partial' if it's a single exercise or clearly mid-session; 'none' if they only asked a question and logged nothing. Judge based on their profile/goal and what a typical full session looks like for them.",
    "Actions you may include:",
...
// ── History drawer ────────────────────────────────────────────────────────────
const HistoryDrawer = ({open,allDays,selectedDay,onSelectDay,onClose,onNav,onExport,onImport,onShare}) => {
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose}
            style={{flex:1,background:"none",border:`1px solid ${T.border}`,color:T.muted,
              ...
              WebkitTapHighlightColor:"transparent"}}>Cancel</button>
          <button onClick={()=>canLog && onLog({
              name:d.name.trim(), calories:+d.calories||0, protein:+d.protein||0,
    ...
    <div onClick={e=>e.stopPropagation()}
      ...
      <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,
        ...
        <div style={{fontSize:12.5,color:T.text,lineHeight:1.8}}>
          ...
          💧 Customizable water goal + hold-to-pick amounts<br/>
          🔗 Source links on branded items<br/>
          👈 Swipe a log entry left to delete<br/>
...
export default function App() {
  ...
  useEffect(()=>{
    const writeSnapshot = () => {
      try {
        _set("nl4_snapshot", JSON.stringify({ days:allDays, goals, meals, workouts, profile, weights, water, barcodes, ts:Date.now() }));
      } catch {}
    };
    writeSnapshot();
  ...
  },[allDays, goals, meals, workouts, profile, weights, water, barcodes]);
  ...
  useEffect(()=>{
    ...
    const checks = {
      calories: t.calories>0 && Math.abs(t.calories-goals.calories)<=100,
      protein:  goals.protein>0 && t.protein>=goals.protein,
      carbs:    goals.carbs>0 && t.carbs>=goals.carbs,
      fat:      goals.fat>0 && t.fat>=goals.fat,
      water:    (goals.water||0)>0 && (water[today]||0)>=goals.water,
    };
    let newlyHit = null;
  ...
  const handleTrainerSend = async () => {
    ...
    try {
      ...
      if (actions.length) {
        ...
        // Celebrate workout logging — big for a complete session, small cheer for partial
        const addedWk = actions.some(a=>a.type==="add_workout");
        if (addedWk) {
          if (wkStatus==="complete") fireCelebration("Workout complete! 🎉 Great session.", true);
          else if (wkStatus==="partial") fireCelebration("Logged! Keep the session going 💪");
          else fireCelebration("Workout logged 💪");
        }
      }
  ...
  const handleExport = () => {
    const blob = {
      ...
      days: allDays, goals, meals, workouts, profile, weights, water, barcodes,
    };
      ...
      {activeTab==="workouts"&&(
        <div style={{flex:1,overflowY:"auto",padding:"12px 14px",WebkitOverflowScrolling:"touch"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,
            background:T.surface,border:`1px solid ${dayWorkouts.length>0?T.info+"88":T.border}`,
            borderRadius:14,padding:"14px",marginBottom:12,
            boxShadow:dayWorkouts.length>0?`${T.glow} ${T.info}44`:"none"}}>
            <div style={{fontSize:30,filter:dayWorkouts.length>0?"none":"grayscale(1) opacity(0.4)",
              animation:dayWorkouts.length>0?"bump .4s ease":"none"}}>🏋️</div>
            <div>
              <div style={{fontSize:22,fontWeight:800,color:dayWorkouts.length>0?T.info:T.muted}}>
