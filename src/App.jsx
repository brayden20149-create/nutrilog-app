...
import { GlowBorder } from "./ui/GlowBorder.jsx";
...
export default function App() {
  ...
  const handleSend = async (skipRepeat = false) => {
    ...
    try {
      ...
      actions.filter(a=>a.type==="save_meal" && a.meal).forEach(a=>{
      ...
      setChatMsgs(prev=>[...prev,{role:"assistant",content:message,actions,sources,mode}]);
    } catch(err) {
      console.error(err);
      setChatMsgs(prev=>[...prev,{
        role:"assistant",
    ...
    <div
      ...
      {showSettings && (
        <SettingsModal current={theme}
          ...
          allDays={allDays} workouts={workouts} goals={goals} water={water} today={todayKey()}
