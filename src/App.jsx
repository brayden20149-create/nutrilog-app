...
// Snapshot of the shipped defaults so "reset" and preset math can always refer back
const T_DEFAULTS = {...T};

// Built-in themes. Each sets a background tone + accent pair; macro colors stay readable.
const THEMES = [
...
// Apply a theme object (preset or custom) by mutating T in place so every T.* reader updates.
const applyTheme = (t) => {
...
const loadTheme = () => { try { return JSON.parse(dualLoadRaw("nl4_theme")||"null"); } catch { return null; } };
const saveTheme = t  => dualSave("nl4_theme", JSON.stringify(t));

// App settings (preferences). Stored as one object.
...
const loadSettings = () => { try { return {...DEFAULT_SETTINGS, ...JSON.parse(dualLoadRaw("nl4_settings")||"{}")}; } catch { return {...DEFAULT_SETTINGS}; } };
const saveSettings = s  => dualSave("nl4_settings", JSON.stringify(s));

// Unit conversion helpers — data is always stored in imperial (lbs / oz)
const toDisplayWeight = (lbs, units) => units==="metric" ? +(lbs*0.453592).toFixed(1) : lbs;
const fromDisplayWeight = (val, units) => units==="metric" ? +(val/0.453592).toFixed(1) : +val;
const weightUnit = (units) => units==="metric" ? "kg" : "lbs";
const toDisplayWater = (oz, units) => units==="metric" ? Math.round(oz*29.5735) : oz;
const fromDisplayWater = (val, units) => units==="metric" ? Math.round(val/29.5735) : +val;
const waterUnit = (units) => units==="metric" ? "mL" : "oz";
...
let HAPTICS_ON = true;
const haptic = (pattern=10) => { try { if (HAPTICS_ON) navigator.vibrate?.(pattern); } catch {} };
...
// ── AI ────────────────────────────────────────────────────────────────────────
// Maps the user's AI-style preference to a system-prompt instruction.
const styleHint = (aiStyle) => {
...
async function callClaude(messages, aiStyle) {
  const SYSTEM = [
...
async function callTrainer(messages, aiStyle) {
  const SYSTEM = [
...
async function callChef(messages, aiStyle) {
  const SYSTEM = [
...
// ── History drawer ────────────────────────────────────────────────────────────
const HistoryDrawer = ({open,allDays,selectedDay,onSelectDay,onClose,onNav,onExport,onImport,onShare,onSettings}) => {
  const days=Object.keys(allDays).filter(d=>allDays[d]?.length>0).sort((a,b)=>b.localeCompare(a));
  const [expanded, setExpanded] = useState(null);
  // Swipe right to dismiss the drawer
  ...
  const onPanelTE = (e)=>{
    ...
    // Require a clear rightward swipe, mostly horizontal, to avoid accidents
...
const ProfileTab = ({ profile, goals, onSave, onApplyGoals, units="imperial" }) => {
...
// Settings / appearance modal — pick a preset theme or set custom colors
// General preferences sub-panel inside Settings
const GeneralSettings = ({ settings, onSet, barcodes, onDeleteBarcode, onClearData }) => {
...
const SettingsModal = ({ current, onApply, onClose, settings, onSet, barcodes, onDeleteBarcode, onClearData, onTry }) => {
  // Seed custom pickers from the current theme or the emerald default
    ...
    <div onClick={onClose}
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        <div style={{display:"flex",gap:6,padding:"12px 14px 0"}}>
          {[["general","General"],["presets","Themes"],["custom","Custom"],["history","History"]].map(([id,lbl])=>(
          ...
          {tab==="general" ? (
            ...
            <>
              <div style={{fontSize:12,color:T.muted,marginBottom:8,lineHeight:1.5}}>
                Pick your own colors. Accent drives buttons & highlights; the others set the
                background tones.
              </div>
...
// Inline version history for use inside Settings (no modal wrapper, just the list)
const VersionHistoryPanel = ({ onTry }) => {
...
const VersionHistoryModal = ({ onClose, onTry }) => {
  const [open, setOpen] = useState(CHANGELOG[0]?.version || null);
  return (
    <div onClick={onClose}
      style={{position:"fixed",inset:0,background:T.overlay,zIndex:520,
...
const WelcomeModal = ({ name, onClose, onTry }) => {
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    HAPTICS_ON = savedSettings.haptics;
  ...
  const updateSetting = (key, value) => {
    setSettings(prev=>{
      ...
      saveSettings(out);
      if (key==="haptics") HAPTICS_ON = value;
      return out;
    });
    haptic(8);
  };
              ...
              <div style={{background:T.surface,borderRadius:14,border:`1px solid ${T.border}`,
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:10}}>
                  ...
                  <span style={{fontSize:12,color:T.muted}}>
                    {toDisplayWeight(latest, settings.units)} {weightUnit(settings.units)} {change!==0 && <span style={{color:change<0?T.accent:T.warn}}>
                      ({change>0?"+":""}{toDisplayWeight(Math.abs(change), settings.units).toFixed(1)} {weightUnit(settings.units)})</span>}
                  </span>
                </div>
                {present.length===1 ? (
