...
export default function App() {
  ...
  useEffect(()=>{
    const writeSnapshot = () => {
      try {
        _set("nl4_snapshot", JSON.stringify({ days:allDays, goals, meals, workouts, ts:Date.now() }));
      } catch {}
    };
    writeSnapshot();
  ...
  const handleTrainerSend = async () => {
    ...
    apiMsgs.push({role:"user",content:text+histBlock});
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex",margin:"10px 14px 0",background:T.surface,
        ...
        {[["chat","💬"],["log","📋"],["meals","🍱"],["train","🏋️"],["week","📅"]].map(([tab,label])=>(
      ...
      {activeTab==="train"&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",padding:"0 14px"}}>
          <div style={{flexShrink:0,padding:"10px 2px 6px"}}>
            <div style={{fontSize:11,color:T.info,letterSpacing:"0.12em"}}>🏋️ STRENGTH COACH</div>
            <div style={{fontSize:11,color:T.muted,marginTop:2}}>
              Technical feedback, cues, and comparison to your history. No fluff.
            </div>
          </div>
          {/* Messages */}
