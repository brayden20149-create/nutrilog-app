...
import { APP_VERSION, T, applyTheme, loadTheme, saveTheme, DEFAULT_SETTINGS, loadSettings, saveSettings, toDisplayWeight, fromDisplayWeight, weightUnit, toDisplayWater, waterUnit, DEFAULT_GOALS, todayKey, isToday, fmtDate, fmtFull, _get, _set, loadAll, saveAll, loadGoals, saveGoals, loadMeals, saveMeals, loadPrograms, savePrograms, loadWorkouts, saveWorkouts, loadStandout, saveStandout, loadWeights, saveWeights, loadWater, saveWater, WATER_STEP, loadBarcodes, saveBarcodes, HAPTICS_ON, haptic, setHapticsOn, DEFAULT_PROFILE, loadProfile, saveProfile, weekStart, addDays, weekDays, dowShort, dayNum, dayHitsGoal, sumDay, computeStreak, mealPerContainer, InfoDot, Ring, Bar, EntryRow, Bubble, HistoryDrawer, MealEditor, ProfileTab, ProgramsTab, Confetti, Toast, BarcodeScanner, ScanConfirm, SettingsModal, WelcomeModal, lookupBarcode, computeHabits, callClaude, callTrainer, callChef } from "./helpers.jsx";
...
export default function App() {
  ...
  // Prefill Coach with a day's exercises and switch to Trainer tab
  ...
  const handleTrainerSend = async () => {
    ...
    try {
      ...
      if (actions.length) {
        ...
        actions.filter(a=>a.type==="save_program" && a.program).forEach(a=>{
          const prog = { ...a.program, id: Date.now().toString(36)+Math.random().toString(36).slice(2),
            days: (a.program.days||[]).map(d=>({...d, id:Date.now().toString(36)+Math.random().toString(36).slice(2),
              exercises:(d.exercises||[]).map(e=>({...e, id:Date.now().toString(36)+Math.random().toString(36).slice(2)}))}))};
          saveProgram(prog);
